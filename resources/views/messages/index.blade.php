...
.back-btn {
    ...
    font-size: 20px;
    cursor: pointer;
    @if(isset($user)) display: block; @else display: none; @endif
}
...
@media (max-width: 768px) {
    ...
    .conversations-sidebar {
        width: 100%;
        @if(isset($user)) display: none; @endif
    }
...
<div class="messaging-container">
    ...
    <div class="conversations-sidebar">
        <div class="conversation-header">
            ...
            <input type="text" class="conversation-search" placeholder="Search..." onkeyup="searchConversations()">
        </div>
        ...
        <div class="conversations-list">
            ...
            @forelse($conversations ?? [] as $conversation)
                <a href="{{ route('messages.conversation', $conversation->other_user->id) }}" class="conversation-item" data-name="{{ $conversation->other_user->pet_name }}" data-breed="{{ $conversation->other_user->pet_breed }}">
    ...
    <script>
        function searchConversations() {
            ...
            conversationItems.forEach(item => {
                ...
                if (name.includes(searchInput) || breed.includes(searchInput)) {
                        ...
                        if (data.success) {
                            ...
                            if (conversationsList) {
                                ...
                                if (convItem) {
                                    ...
                                    if (small) {
                                        const displayText = data.message.message.length > 30 ? data.message.message.substring(0, 30) + '...' : data.message.message;
        ...
        // Polling fallback for realtime messages (used when Echo/Pusher isn't available)
        ...
        function updateLastMessageIdFromDOM() {
            ...
            const ids = Array.from(messagesArea.querySelectorAll('[data-message-id]')).map(el => parseInt(el.getAttribute('data-message-id') || '0'));
        ...
        function startPolling(userId) {
            ...
            pollInterval = setInterval(async () => {
                try {
                    const resp = await fetch(`/api/messages/${userId}`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
                    if (!resp.ok) return;
                            ...
                            if (msg.sender_id == userId) {
                                const messageHtml = `
                                                                                                                    <div class="message-received" data-message-id="${msg.id}">
                                                                                                                        <img src="${msg.sender.avatar || '/assets/usericon.png'}" style="width: 35px; height: 35px; object-fit: cover; border-radius: 50%;">
                                                                                                                        <div class="message-bubble-received">
    ...
    <script>
        // Attach message listeners when Echo is available (bundled via Vite)
        (function () {
            ...
            const attachListeners = () => {
                try {
                    ...
                    if (typeof window.Echo === 'undefined' || !window.Echo.private) {
                        // Echo not ready yet, try again shortly
                        ...
                        .listen('MessageSent', (e) => {
                            try {
                                ...
                                if (convItem) {
                                    ...
                                    if (smalls.length > 0) {
                                        const displayText = messageText ? (messageText.length > 30 ? messageText.substring(0, 30) + '...' : messageText) : 'Sent a message';
                                        smalls[smalls.length - 1].textContent = senderId == {{ auth()->id() }} ? 'You: ' + displayText : '{{ auth()->user()->pet_name }}: ' + displayText;
                                    }
                                    const firstConv = conversationsList.querySelector('.conversation-item');
                                    if (firstConv && convItem !== firstConv) {
                                        conversationsList.insertBefore(convItem, firstConv);
                                ...
                                if (receiverInput && parseInt(receiverInput.value) === parseInt(senderId)) {
                                    if (messagesArea) {
                                        const messageHtml = `
                                                                                                                            <div class="message-received">
                                                                                                                                <img src="${e.sender?.avatar || '/assets/usericon.png'}" style="width: 35px; height: 35px; object-fit: cover; border-radius: 50%;">
                                                                                                                                <div class="message-bubble-received">
                                    ...
                                    try {
                                        ...
                                        window.showToast({
                                            ...
                                            body: (e.message || '').length > 80 ? (e.message || '').substring(0, 80) + '...' : (e.message || ''),
